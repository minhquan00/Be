...
// Hàm randomElement được định nghĩa trước để tránh vấn đề hoisting
function randomElement(element) {
    if (!element || element.length === 0) {
        throw new Error("User agent list is empty or undefined");
    }
...
// Xóa định nghĩa lặp lại của Array.prototype.remove
...
async function solveCaptcha(browser, page, browserProxy, waitTime = Math.floor(Math.random() * 8) + 4) {
    try {
...
async function launchBrowserWithRetry(targetURL, browserProxy, attempt = 1, maxRetries = 2) {
    let browser;
    const userAgent = randomElement(userAgents); // Chọn userAgent ngẫu nhiên cho mỗi trình duyệt
    const options = {
        headless: true,
        args: [
    ...
    try {
        ...
        await spoofFingerprint(page, userAgent); // Truyền userAgent vào spoofFingerprint
        ...
        while (captchaAttempts < maxCaptchaAttempts) {
            await solveCaptcha(browser, page, browserProxy, captchaAttempts === 0 ? Math.floor(Math.random() * 8) + 4 : 17);
    ...
    try {
        ...
        if (response) {
            ...
            if (!response.cookies) {
                ...
                let browser = await puppeteer.launch({ /* options như trong launchBrowserWithRetry */ });
                const [page] = await browser.pages();
                await spoofFingerprint(page, randomElement(userAgents));
                await page.goto(targetURL, { waitUntil: "domcontentloaded" });
                await solveCaptcha(browser, page, browserProxy, 17);
                await browser.close();
                return;
            }
            cookieCount++;
            ...
            if (!fs.existsSync('flood.js')) {
                colored(colors.COLOR_RED, "[ERROR] flood.js not found");
                done(new Error("flood.js not found"), { task });
                return;
            }
